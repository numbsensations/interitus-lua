...
local function brute_side()
    ...
    local testfix = disablerI and ignore_brute_fix

    if not ui.get(menu.bruteTab.enable) or not func.includes(ui.get(menu.bruteTab.additions), "Flip Side") or disabler or not disablerm or not testfix or antibrute_data.miss_counter == 0 then 
...
local function get_desync(is_for_skeet)
    ...
    if antibrute_data.miss_counter == 0 or ui.get(menu.bruteTab.phases) == 0 or disabler or testfix then       
        desync = ui.get(aaBuilder[vars.pState].bodyYawSlider)
...
--            ui.set(refs.yaw[2], "Custom")
        ...
        elseif ui.get(aaBuilder[vars.pState].yawCondition) == "Random" then
            ui.set(refs.yaw[2], math.random(ui.get(aaBuilder[vars.pState].yawLeft) ,ui.get(aaBuilder[vars.pState].yawRight)))
        ...
        elseif ui.get(aaBuilder[vars.pState].yawCondition) == "Hold" and not ui.get(aaBuilder[vars.pState].bodyYawAllowOverride) and ui.get(menu.bruteTab.enable) then
            ...
            elseif ui.get(aaBuilder[vars.pState].bodyYaw) == "Random" then
                ui.set(refs.bodyYaw[1],"Jitter")
                ui.set(refs.bodyYaw[2], math.random(get_desync_value, -get_desync_value))
            elseif ui.get(aaBuilder[vars.pState].desync_type) == "Interitus" then
                ui.set(refs.bodyYaw[2], 0)
                local get_desync_value = get_desync(false)
                apply_desync(cmd, get_desync_value)
...
client.set_event_callback('setup_command', function()
    ...
    if not dt_charged
    and threat and (entity.get_classname(weapon) == "CWeaponSSG08" or entity.get_classname(weapon) == "CWeaponAWP") then
...
local function menu_handler()
    ...
    for i = 1, #vars.aaStates do
        ...
        ui.set_visible(aaBuilder[i].bodyYaw, vars.activeState == i and isBuilderTab and stateEnabled and isEnabled  and isLogicBuilder)
        ui.set_visible(aaBuilder[i].bodyYawSlider, vars.activeState == i and ui.get(aaBuilder[i].bodyYaw) ~= "Off" and isBuilderTab and stateEnabled and isEnabled  and isLogicBuilder)
        ui.set_visible(aaBuilder[i].bodyYawAllowOverride, vars.activeState == i and ui.get(aaBuilder[i].bodyYaw) ~= "Off" and isBuilderTab and stateEnabled and isEnabled  and isLogicBuilder and (ui.get(aaBuilder[i].yawCondition) == "Hold" or ui.get(aaBuilder[i].yawCondition) == "Slow"))
